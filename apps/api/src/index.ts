...
import { VitalData } from "@lucid/types/tracker";
...
import { Filter, LucidEvent, Path } from "./type";
...
app.get("/", async (c) => {
    ...
    try {
        ...
        const filters = JSON.parse(queries.data.filter) as Filter<LucidEvent>[];
        ...
        filters.length &&
            filters.forEach((f) => {
                ...
                lastEvents = filter(lastEvents as LucidEvent[])
                    .where(f.key, f.operator, f.value)
                    .execute();
            });

        const insightData = getInsight(events as LucidEvent[], lastEvents as LucidEvent[]);
        const tableData = getTablesData(
            events as LucidEvent[],
            startDateObj,
            endDateObj,
            timeZone,
        );
...
app.post("/send-ingest", async (c) => {
    ...
    try {
        ...
        console.log(tack - tick, "ms taken to query");
        const insightData = getInsight(events as LucidEvent[], lastEvents as LucidEvent[]);
        const tableData = getTablesData(events as LucidEvent[], startDate, endDate, data.timezone);
        await mail
            .sendIngestedEmail({
                to: data.email,
                website: website,
