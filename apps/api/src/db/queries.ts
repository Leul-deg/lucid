...
export const customEventsQuery = (startDate: string, endDate: string, websiteId: string) =>
    `select * from lucid.event WHERE timestamp >= '${startDate}' AND timestamp <= '${endDate}' AND websiteId = '${websiteId}' AND event != 'hits' AND event != 'vitals'`;
...
export const vitalsQuery = (startDate: string, endDate: string, websiteId: string) => {
    ...
    return `select id, sessionId, visitorId, properties, timestamp from lucid.event WHERE timestamp >= '${startDate}' AND timestamp <= '${endDate}' AND websiteId = '${websiteId}' AND event = 'vitals'`;
};
    ...
    return {
        ...
        clickhouse: async () => {
            const res = await client
                ...
                .then(async (res) => (await res.json()) as LucidEvent[]);
...
export function lucidDb(db: "sqlite" | "clickhouse") {
    return {
        async insertEvent(
            data: LucidEvent & {
                payload?: string;
