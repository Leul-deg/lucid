...
import { TrackView } from "@lucid/tracker/react";
...
async function getGitHubContributors() {
  try {
    const response = await fetch(
      "https://api.github.com/repos/lucid/lucid/contributors",
      {
        next: {
          revalidate: 60,
        },
...
async function getGitHubStars() {
  try {
    const response = await fetch("https://api.github.com/repos/lucid/lucid", {
      next: {
        revalidate: 60,
      },
    });
...
async function getGitHubForks() {
  return await fetch("https://api.github.com/repos/lucid/lucid/forks", {
    method: "GET",
    redirect: "follow",
    next: {
      revalidate: 60,
...
export default async function IndexPage() {
  ...
  const npm = await getPackageInfo("@lucid/tracker");
      ...
      >
        <footer className=" border-t !w-full py-10 my-9 justify-between flex items-center">
          ...
          <div className=" flex items-center gap-2">
            <Link href="https://twiiter.com/lucid_io">
              <TwitterXIcons width={15} height={15} size={15} />
            </Link>
            <Link href="https://github.com/lucid">
              <svg
                width="18"
                height="18"
                viewBox="0 0 16 16"
            ...
            <Link href="https://t.me/lucid_community">
              <svg
                viewBox="0 0 24 24"
                className=" w-6 h-6"
                fill="none"
