    ...
    return {
        clickhouse: async () => {
            const sessionsCount = await client.query({
                query: `select visitorId as id from lucid.event where websiteId='${websiteId}' AND timestamp >= '${before24Hour.toISOString().slice(0, 19).replace("T", " ")}'`,
                format: "JSONEachRow",
            });
...
const getTotalEventsCount = async (websiteIds: string[], startDate: Date, endDate: Date) => {
    return {
        clickhouse: async () => {
            const sessionsCount = await client.query({
                query: `select * from lucid.event where websiteId IN (${websiteIds.map(w => `'${w}'`)}) AND timestamp >='${startDate.toISOString().slice(0, 19).replace("T", " ")}' AND timestamp <='${endDate.toISOString().slice(0, 19).replace("T", " ")}'`,
                format: "JSONEachRow"
