...
export function getVisitorId() {
    const getId = () => localStorage.getItem("lucid-id");
    if (window.llc.consent === "denied" && !getId()) {
        return "";
    } else {
        window.llc.consent = "granted";
        if (!getId()) {
            window.localStorage.setItem("lucid-id", guid());
        }
        return getId();
    }
}
...
export function getUrl() {
    if (typeof process === "undefined") {
        return "https://api.lucid.io";
    }
    if (process.env.NEXT_PUBLIC_LUCID_URL || process.env.LUCID_URL) {
        const url = process.env.NEXT_PUBLIC_LUCID_URL ?? process.env.LUCID_URL;
        return `${url}/api/lucid`;
    }
    return "https://api.lucid.io";
}
...
export const parseHost = (url: string) => {
    ...
    if (url === "https://lucid.io" || url === "https://www.lucid.io") {
        return "https://lucid.io/api/lucid";
    } else {
        return url;
    }
};
...
export const setSessionStartTime = (time: number) => {
    sessionStorage.setItem("lucid-session-start-time", time.toString());
};

export const getSessionDuration = () => {
    const startTime = sessionStorage.getItem("lucid-session-start-time");
    if (startTime) {
